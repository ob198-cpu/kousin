...
function apiCreateOrUpdateAuditWorkspace(request) {
  ...
  try {
    ...
    var pdf = renewalPdfExportAndSave_(
      resolved.file,
      "audit-workspace:" + fiscalYear,
      settings
    );
    complianceEnsureServerAudit_({
      actor: authorization.email,
      scopeKey: "audit-workspace:" + fiscalYear,
      kind: "auditWorkspace",
      hash: contentHash,
      fileId: resolved.file.getId() + ":" + pdf.fileId,
      action: resolved.created ? "AUDIT_WORKSPACE_CREATE" : "AUDIT_WORKSPACE_UPDATE"
    });
    return {
      success: true,
      ready: true,
      ...
      message: fiscalYear + "年度の全体監査資料4シートとPDFを同じファイルIDで更新しました。",
      warnings: finance ? [] : ["正式会計台帳が未設定のため、講習料金収納記録は見出しだけです。"]
    };
  } catch (error) {
...
function auditWorkspaceFileName_(fiscalYear) {
  return "更新講習_全体監査資料_" + fiscalYear + "年度";
}

function auditWorkspaceDescription_(fiscalYear) {
  return RENEWAL_AUDIT_WORKSPACE.FORMAT + "\n" + artifactCanonicalJson_({ fiscalYear: fiscalYear });
}

function auditWorkspaceResolve_(folder, fiscalYear, allowedEmails) {
  ...
  if (storedId) {
    ...
    var matches = [];
    var iterator = folder.getFilesByName(auditWorkspaceFileName_(fiscalYear));
  ...
  if (!file) {
    var createdItem = artifactCreateSpreadsheetInFolder_(
      auditWorkspaceFileName_(fiscalYear), folder, "全体監査資料", allowedEmails, false
    );
    file = createdItem.file;
    created = true;
    file.setDescription(auditWorkspaceDescription_(fiscalYear));
  }
  if (artifactText_(file.getDescription()) !== auditWorkspaceDescription_(fiscalYear)) {
    throw new Error("全体監査資料の管理識別情報が一致しないため上書きしません。");
  }
  artifactAssertReusableDriveItem_(file, folder.getId(), "全体監査資料", allowedEmails);
  ...
  if (created) {
    artifactClearPublishedDriveAttempt_(
      artifactDriveAttemptOperation_(
        "CREATE", "", auditWorkspaceFileName_(fiscalYear),
